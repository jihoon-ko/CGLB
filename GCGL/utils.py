    ...
    def __init__(self, args):
        if args['dataset'] in ['Tox21','Tox21-tIL']:
            ...
            train_set, val_set, test_set = RandomSplitter.train_val_test_split(
                ...
                frac_test=args['frac_test'], random_state=args['random_seed'])
            args['n_train_examples'] = len(train_set)
        elif args['dataset'] in ['QM7bDataset','QM7bDataset-tIL']:
            ...
            dataset = MUV(smiles_to_graph=partial(args['smiles_to_graph'], add_self_loop=True),
                          node_featurizer=args.get('node_featurizer', None),
                          edge_featurizer=args.get('edge_featurizer', None),
                          # load=True,
                          cache_file_path='./' + args['exp'])
            train_set, val_set, test_set = RandomSplitter.train_val_test_split(
                dataset, frac_train=args['frac_train'], frac_val=args['frac_val'],
                frac_test=args['frac_test'], random_state=args['random_seed'])
        elif args['dataset'] == 'TencentAlchemyDataset':  # may try complete graph
            from dgllife.data import TencentAlchemyDataset
            dataset = TencentAlchemyDataset(smiles_to_graph=partial(args['smiles_to_graph'], add_self_loop=True),
                                            node_featurizer=args.get('node_featurizer', None),
                                            edge_featurizer=args.get('edge_featurizer', None),
                                            # load=True,
                                            cache_file_path='./' + args['exp'])
            ...
            dataset = PCBA(smiles_to_graph=partial(args['smiles_to_graph'], add_self_loop=True),
                           node_featurizer=args.get('node_featurizer', None),
                           edge_featurizer=args.get('edge_featurizer', None),
                           # load=True,
                           cache_file_path='./' + args['exp'])
            ...
            if args['method'] not in ['jointtrain', 'jointreplay'] or not args['clsIL']:
            ...
            elif args['method'] is 'jointtrain':
                train_ids_task, val_ids_task, test_ids_task = [], [], []
                for clss in args['tasks']:
                    for cls in clss:
                        ids = ids_per_cls[cls]
                ...
                test_set.append(Subset(dataset, test_ids_task))
                args['n_train_examples'] = len(train_ids_task)
            elif args['method'] is 'jointreplay':
                # train_ids_task, val_ids_task, test_ids_task = [], [], []
        ...
        elif args['dataset'] == 'MiniGCDataset':
            from dgl.data import MiniGCDataset
            dataset = MiniGCDataset(8000, 16, 17, seed=0)
            for g in dataset.graphs:
                g.ndata['h'] = torch.ones(g.num_nodes(), 1)
            n_per_cls = np.array([(dataset.labels == i).sum() for i in range((dataset.labels.max().int().item() + 1))])
            selected_clss = (n_per_cls > 2).nonzero()[0].tolist()  # at least 3 examples for train val test splittings
            ids_per_cls = {i: (dataset.labels == i).nonzero().view(-1).tolist() for i in selected_clss}
            args['n_per_cls'] = {i: len(ids_per_cls[i]) for i in ids_per_cls}
            args['tasks'] = []
            while len(selected_clss) >= args['n_cls_per_task']:
            ...
            frac_list = np.asarray([args['frac_train'], args['frac_val'], args['frac_test']])
            train_set, val_set, test_set = [], [], []
            if args['method'] is not 'jointtrain':
                for clss in args['tasks']:
                    train_ids_task, val_ids_task, test_ids_task = [], [], []
                    for cls in clss:
                        ids = ids_per_cls[cls]
...
def load_model(args):
    ...
    if args['backbone'] == 'HPNs':
        from Backbones.gcn_predictor import HPNsPredictor
        model = HPNsPredictor(in_feats=args['node_featurizer'].feat_size(),
                              hidden_feats=args['gcn_hidden_feats'],
                              predictor_hidden_feats=args['predictor_hidden_feats'],
                              n_tasks=args['n_tasks'], n_protos=args['n_protos'], t_protos=args['t_protos'],
                              AFE_budget=args['AFE_budget'], args=args)
